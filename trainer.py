...
class RankCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, num_neg: int = 1, num_test_neg: int = 1):
        """
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor, stage='train'):
        ...
        if stage == 'train':
            ...
            logits = y_pred[::(self.num_test_neg + 1), :]
            labels = y_true[::(self.num_test_neg + 1), :]
            for neg_idx in range(self.num_test_neg):
                neg_logits = y_pred[(neg_idx + 1)::(self.num_test_neg + 1), :]
                neg_labels = y_true[(neg_idx + 1)::(self.num_test_neg + 1), :]
                logits = torch.cat((logits, neg_logits), dim=-1)
                labels = torch.cat((labels, neg_labels), dim=-1)
            return -torch.mean(
                torch.sum(
...
class RankHingeLoss(nn.Module):
    ...
    def __init__(self, num_neg: int = 1, num_test_neg: int = 1, margin: float = 1.,
                 reduction: str = 'mean'):
        """
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor, stage='train'):
        ...
        if stage == 'train':
            ...
            y_pos = y_pred[::(self.num_test_neg + 1), :]
            y_negs = []
            for neg_idx in range(self.num_test_neg):
                neg = y_pred[(neg_idx + 1)::(self.num_test_neg + 1), :]
                y_negs.append(neg)
            losses = [F.margin_ranking_loss(y_pos, neg, torch.ones_like(y_pos), margin=self.margin) for neg in y_negs]
...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # self._criterions = [RankHingeLoss(num_neg=config.num_neg, num_test_neg=config.num_test_neg)]
        self._criterions = [RankCrossEntropyLoss(num_neg=config.num_neg, num_test_neg=config.num_test_neg)]
        ...
        self._load_path(checkpoint, config.save_path)
    ...
    #             # if self._epoch <= 10:
    #             #     y_pred = self._model.forward1(inputs_).cpu().numpy()
    #             # else:
    #             #     y_pred = self._model.forward2(inputs_).cpu().numpy()
    #             y_pred = self._model.forward(inputs_).cpu().numpy()
    #             y_pred_all = y_pred_all + list(y_pred)
    #         y_pred_all = np.array(y_pred_all)

    #     return y_pred_all.squeeze(axis=-1)
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                userId = inputs['userId'].cpu().numpy()
                ...
                for sg in score_df_group:
                    self.score_fun(sg[1]['record_id'].values, sg[1]['userId'].values, sg[1]['id_left'].values, sg[1]['id_right'].values, sg[1]['label'].values, sg[1]['preds'].values)
            self._constant.logger.info(f'Valid Loss-{valid_loss_a.avg:.3f}]:')
            # self._constant.tensorboard_logger.add_scalar('Avg/valid_loss', valid_loss_a.avg, self._epoch)
            self._model.train()
            ...
            self.scores_df = pd.DataFrame(self.results_score_list,
                                          columns=['record_id', 'userId', 'id_left', 'id_right', 'labels', 'scores_ie'])
    ...
    def score_fun(self, record_ids, userId, id_lefts, id_rights, labels, preds):
        sort_idx = np.argsort(id_rights)
        self.results_score_list.append(
            [record_ids[0], userId[0], id_lefts[0], id_rights[0], labels[sort_idx], preds[sort_idx]])
